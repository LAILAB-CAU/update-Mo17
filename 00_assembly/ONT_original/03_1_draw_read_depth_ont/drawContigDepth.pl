...
my ($pseudomolecule,$highlight,$depth1,$depth2) = @ARGV;
...
my %hashDepth2;
open NEW,"$depth2" or die;
while(<NEW>){
	chomp;
	my @array = split /\s+/;
    if($array[3] >=300){
        $hashDepth2{$array[0]}{"$array[1]\t$array[2]"} = 300;
    }
    else{
        $hashDepth2{$array[0]}{"$array[1]\t$array[2]"} = $array[3];
    }
}
close NEW;

my %hashBed;
open NEW, "$highlight" or die;
while(<NEW>){
    chomp;
    my @array = split /\s+/;
...
foreach my $key(sort  keys %hash_scaffolds){
    ...
    foreach my $loc(keys %{$hashBed{$seqName}}){
        ...
        if($start >= $regionStart and $end <= $regionEnd){
            $svg->rectangle(x=>50+($start - $regionStart)/$scale,y=>$y_loc+1,width=>$length/$scale,height=>9,fill=>"$hashBed{$seqName}{$loc}",stroke=>'none',style => {'stroke-width'   => '0.001'});
        }
    }
    foreach my $loc(keys %{$hashDepth2{$seqName}}){
        ...
        my $length = $end-$start;
        if($start >= $regionStart and $end <= $regionEnd){
            $svg->rectangle(x=>50+($start - $regionStart)/$scale,y=>$y_loc-12-$hashDepth2{$seqName}{$loc}/5,width=>$length/$scale,height=>$hashDepth2{$seqName}{$loc}/5,fill=>"grey",stroke=>'none',style => {'stroke-width'   => '0.001'});
        }
    }
	foreach my $loc(keys %{$hashDepth{$seqName}}){
        ...
        my $length = $end-$start;
        if($start >= $regionStart and $end <= $regionEnd){
            $svg->rectangle(x=>50+($start - $regionStart)/$scale,y=>$y_loc-12-$hashDepth{$seqName}{$loc}/5,width=>$length/$scale,height=>$hashDepth{$seqName}{$loc}/5,fill=>"black",stroke=>'none',style => {'stroke-width'   => '0.001'});
        }
    }

	$svg->line(x1=>45,x2=>45,y1=>$y_loc-12,y2=>$y_loc-12,stroke=>'black','stroke-width'=> '0.35');
