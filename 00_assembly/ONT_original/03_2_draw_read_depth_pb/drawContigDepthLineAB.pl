...
while(<NEW>){
	...
	my $contigSeq = $array[0] =~ s/Contig//r;
	$hash_sca_seq{"$array[0]\t$array[1]\t$array[2]"} = $contigSeq;
}
close NEW;
...
while(<NEW>){
	...
	$hashDepth{$array[0]}{"$array[1]\t$array[2]"} = "$depth1\t$depth2";
}
close NEW;
...
while(<NEW>){
	...
	$hashDepth2{$array[0]}{"$array[1]\t$array[2]"} = "$depth1\t$depth2";
}
close NEW;
...
foreach my $key(sort {$hash_sca_seq{$a} <=> $hash_sca_seq{$b}} keys %hash_scaffolds){
    ...
    if($countInPicture == 0){
        ...
        $svg = SVG->new(width=>3500, height=>100*20 + 100);
    }
    my $step_len;
	...
	$svg->line(x1=>45,x2=>45,y1=>$y_loc-12,y2=>$y_loc-12-100/5,stroke=>'black','stroke-width'=> '0.35');
	for(my $depthLabel=0;$depthLabel<=100;$depthLabel+=50){
