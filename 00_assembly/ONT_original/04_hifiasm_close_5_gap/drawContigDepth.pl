...
foreach my $key(sort  keys %hash_scaffolds){
    ...
    my $countInPicture = $count % 2;
    my $thePictureSeq = int($count/2);
    ...
    if($countInPicture == 0){
        ...
        $svg = SVG->new(width=>3500, height=>100*2 + 100);
    }
    my $step_len;
    ...
    foreach my $loc(keys %{$hashBed{$seqName}}){
        ...
        if($start <= $regionEnd and $end >= $regionStart){
			...
			$svg->rectangle(x=>50+($draw_s - $regionStart)/$scale,y=>$y_loc+1,width=>($draw_e - $draw_s)/$scale,height=>9,fill=>"$hashBed{$seqName}{$loc}",stroke=>'none',style => {'stroke-width'   => '0.001'});
		}
    }
