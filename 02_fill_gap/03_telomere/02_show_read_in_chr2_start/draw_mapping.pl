...
foreach my $key(sort {$hash_karyotypes{$b} <=> $hash_karyotypes{$a} }keys %hash_karyotypes){
    ...
    foreach my $loc(keys %{$hashBed{$read}}){
        ...
        if($start >= $old_regionStart and $end <= $old_regionEnd){
            $svg->rectangle(x=>50+($start-$old_regionStart)/$scale,y=>$y_loc+1,width=>$length/$scale,height=>9,fill=>"$hashBed{$read}{$loc}",stroke=>'none',style => {'stroke-width'   => '0.001'});
        }
    }
    ...
    foreach my $loc(keys %{$hashBed{$read}}){
        ...
        if($start >= $regionStart and $end <= $regionEnd){
            $svg->rectangle(x=>50+($start-$regionStart)/$scale,y=>$y_loc + 1 + 70 ,width=>$length/$scale,height=>9,fill=>"$hashBed{$read}{$loc}",stroke=>'none',style => {'stroke-width'   => '0.001'});
        }
    }
